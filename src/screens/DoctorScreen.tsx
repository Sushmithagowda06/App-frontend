...
const DoctorScreen = () => {
  ...
  const fetchDoctors = async () => {
    console.log("DoctorScreen: Fetching doctors...");
    setLoading(true);
    setError(null);
    try {
      const data = await doctorApi.getAll();
      console.log("DoctorScreen: Raw API response:", data);
      
      // Handle different response formats
      const doctorsArray = Array.isArray(data) ? data : (data as any)?.doctors ?? (data as any)?.results ?? [];
      console.log("DoctorScreen: Processed doctors array:", doctorsArray.length, "doctors");
      
      setDoctors(doctorsArray);
    } catch (err: any) {
      console.error("DoctorScreen: Error fetching doctors:", err);
  ...
  useEffect(() => {
    console.log("DoctorScreen: useEffect called - fetching doctors");
    fetchDoctors();
  }, []);

  if (loading) {
    console.log("DoctorScreen: Loading doctors...");
    return (
  ...
  if (doctors.length === 0 && !loading) {
    console.log("DoctorScreen: No doctors found and not loading");
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.centered}>
  ...
  console.log("DoctorScreen: Rendering with", doctors.length, "doctors");
      ...
      <View style={[styles.pageInner, isDesktopLike && styles.pageInnerDesktop]}>
        ...
        {doctors.length > 0 ? (
          <FlatList
            ...
            renderItem={({ item }) => {
              console.log("DoctorScreen: Rendering doctor card for", item.name);
              return (
                <DoctorCard
                  doctor={item}
...
const styles = StyleSheet.create({
  ...
  debugText: { fontSize: 12, color: "red", fontWeight: "bold", marginBottom: 10 },
});
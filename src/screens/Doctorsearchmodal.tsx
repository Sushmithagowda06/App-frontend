...
const DoctorSearchModal: React.FC<Props> = ({ visible, onClose, initialSpecialization }) => {
      ...
      .then((data) => {
        ...
        if (initialSpecialization) {
          ...
          const match = data.find(
            (d) => d.specialty.toLowerCase() === initialSpecialization.toLowerCase()
          );
          if (match) {
            setActiveChip(match.specialty);
            setQuery("");
          } else {
  ...
  const results: Doctor[] = (() => {
    if (activeChip) {
      return doctors.filter((d) => d.specialty === activeChip);
    }
    ...
    return doctors
      .map((doc) => ({
        ...doc,
        score:
          getScore(doc.specialty, query) * 2 + getScore(doc.name, query),
      }))
    ...
    switch (item.type) {
          ...
          >
            ...
            <View style={styles.chipRight}>
              <Text style={[styles.chipCount, isActive && styles.chipCountActive]}>
                {doctors.filter((d) => d.specialty === item.spec).length}
              </Text>
              <Ionicons
      ...
      case "doctor": {
        ...
        return (
          <TouchableOpacity
            style={styles.card}
            onPress={() => handleChip(doc.specialty)}
            activeOpacity={0.75}
          >
            ...
            <View style={styles.cardInfo}>
              ...
              <View style={styles.specRow}>
                ...
                <Text style={styles.cardSpec}>{doc.specialty}</Text>
              </View>
